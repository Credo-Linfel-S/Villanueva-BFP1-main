...
const History = () => {
      ...
      const transformedData = (data || []).map((person) => {
        ...
        return {
          ...
          years_of_service: calculateYearsOfService(
            person.date_hired,
            person.retirement_date || person.updated_at
          ),
  ...
  const filteredRecords = historyRecords.filter((record) => {
    ...
    if (filterType !== "all") {
      ...
      if (filterTypeLower === "retirement" && !recordStatus.includes("retire"))
        return false;
      if (filterTypeLower === "resignation" && !recordStatus.includes("resign"))
        return false;
      if (
        filterTypeLower === "equipment completion" &&
        !recordStatus.includes("equipment")
      )
        return false;
    }
    ...
    <div className={styles.container}>
      ...
      <div className={`main-content ${isSidebarCollapsed ? "collapsed" : ""}`}>
              ...
              <p className={styles.statNumber}>
                {
                  historyRecords.filter(
                    (r) => r.status === "Equipment Completed"
                  ).length
                }
              </p>
            </div>
        ...
        <div className={styles.instructions}>
          ...
          <ol>
            <li>
              Go to <strong>Clearance System</strong>
            </li>
            <li>
              Initiate a clearance request (Retirement, Resignation, or
              Equipment Completion)
            </li>
            <li>
              Approve the request by clicking "Approve" (status changes to
              "Completed")
            </li>
            <li>
              The personnel will automatically appear here with appropriate
              status
            </li>
          </ol>
        </div>
